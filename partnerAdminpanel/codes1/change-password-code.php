...
if (isset($_POST['change_password']) && isset($_SESSION['U_Name_partner']) && $_SESSION['U_Name_partner']!="") {
    ...
    $services_name_query = "select * from partners_registration where partners_id='$partner_admin_id' and roles='partners' limit 1";
      ...
      if ($codes) { // if user exists
        ...
        if ($old_password!==$codes['partners_password']) { 
    ...
    $partners='partners';
        ...
        if (count($errors) == 0) {
            ...
            $stmt = $db->prepare("update partners_registration set partners_password=? where partners_id=? and partners_username=? and roles=?");
