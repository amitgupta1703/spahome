...
if(isset($_POST['franchiseresendOtps'])){
    if(!isset($_COOKIE['f_bpar'])){
        echo "<script> window.location.assign('franchise'); </script>";
    }
        
    $otpSend; 
    $mobileNo=mysqli_real_escape_string($db, $_POST['franchisemobileNo']);
    $oldOTP; 
    $pIds='';
    $otp='';
    $id;
    ...
    $otp=(generateNumericOTP($n)); 
    $getIds='';
    $getIds=$_COOKIE['f_bpar']; 
    $pIds=explode("-",$getIds)[1]; 
    $status='Not Verify'; 
    $stmt = $db->prepare("update franchise set otp=? where id=? and phone_verification_status=?");
    $rc=$stmt->bind_param("sss", $otp,$pIds,$status); 
    $rc=$stmt->execute(); 
    if($rc==true){ 
        $setpid="hGME2322x-".$pIds."-h73232";
        setcookie("f_bpar",$setpid,time()+60*60*24*30);
        array_push($successMsg,"Otp resend successfully");  
        sendOtp($otp,$mobileNo);
...
if (mysqli_num_rows($results) >0) {  
    while($row = mysqli_fetch_row($results)){
        ...
        if($addressValue==$city){
            //echo 'if:: '.$city." ::: ".$addressValue;
            $count=1;
             
        }  
    } 
    if($count==0){ 
        array_push($errors, "Please select valid city");
    }
}
...
if (count($errors) == 0) { 
  ...
  //echo 'abc: '.$result;
   ...
   $locality=""; 
   $pincode="";
   array_push($successMsg,"Address added successfully!!");
